...
/**
 * Destroys local administrative state and safely redirects client.
 */
function handleAdminLogout() {
  // Clear the authentication payload
  localStorage.removeItem('tk_admin_token');

  // Clear any structural memory caching flags if used
  localStorage.removeItem('tk_user_role'); 
  sessionStorage.clear();

  // Route back to the authentication entry point
  window.location.replace('/Admin/login.html');
}

/**
 * Interactive logout handler with validation checks.
 * @param {boolean} confirmFirst - Demands intentional user confirmation.
 */
function initiateSessionTermination(confirmFirst = true) {
  if (confirmFirst) {
    const wantsToLogout = confirm('Are you sure you want to log out of the admin portal?');
...
// Global Event Initialization Wrapper
document.addEventListener('DOMContentLoaded', () => {
  const logoutBtn = document.getElementById('admin-logout-btn');
  if (logoutBtn) {
    logoutBtn.addEventListener('click', (e) => {
      e.preventDefault();
      initiateSessionTermination(true);
    });
  }
});